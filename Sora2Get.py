...
async def _fetch_with_browser(browser, url: str) -> Optional[str]:
...
async def main_loop(single_run: bool = False):
    import math
    ...
    try:
        while RUNNING:
            try:
                ...
                if not latest and browser:
                    latest = await _fetch_with_browser(browser, QIITA_COMMENTS_URL)
...
if __name__ == "__main__":
    ...
    asyncio.run(main_loop(single_run=args.once))
